...
export const comparisonDetailPages: ComparisonDetailPage[] = [
  ...
  // ========== JOBBER VS HOUSECALL PRO ==========
  {
    ...
    faqs: [
      ...
      { question: "Which has better QuickBooks integration?", answer: "For most service contractors under $500K, Jobber's QuickBooks integration is sufficient — it syncs invoices, payments, and customers without extra configuration. HouseCall Pro's sync includes slightly more field mapping options (useful if your bookkeeper uses job classes or custom fields), but that additional complexity matters only at 3+ techs with a dedicated bookkeeper. Pick based on the rest of the platform, not the QuickBooks integration." },
  ...
  // ========== BUILDERTREND VS JOBTREAD ==========
  {
    ...
    faqs: [
      { question: "Is JobTread as good as Buildertrend?", answer: "For residential builders doing 5–20 projects per year, JobTread covers the core workflow — estimating, scheduling, change orders, client portal, invoicing — as well as Buildertrend and at a lower entry price. Buildertrend becomes the better choice once you pass roughly 20 projects per year or need its selections management depth and 100+ integrations. Below that threshold, JobTread is the stronger value." },
  ...
  // ========== STACK VS BLUEBEAM ==========
  {
    ...
    comparisonTable: [
      ...
      },
      {
        ...
        rows: [
          { label: "Multi-user markup", tool1: "Limited", tool2: "Studio Sessions — up to 500 concurrent users" },
          { label: "PDF annotation tools", tool1: "Basic", tool2: "Full markup suite: callouts, measurements, stamps, custom layers" },
  ...
  // ─── Raken vs Fieldwire ────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      { heading: "Decision Framework", content: "<p><strong>GCs on public or federal work:</strong> Start with Raken. Daily logs with auto-populated NOAA weather data and structured manpower records are the documentation backbone for delay claims and certified payroll compliance. Add Fieldwire once you have Raken running. <strong>Specialty subs:</strong> Raken's subcontractor daily reporting portal lets subs log their own daily reports without relying on the GC's platform — critical for T&M documentation and delay protection. <strong>Fieldwire at $54/user/month</strong> is justified for GCs managing 10+ active field staff across multiple concurrent projects where plan coordination and punch list management are the primary field problem. Below that project volume, Raken's free tier or $15/user paid plan handles most documentation needs for less. <strong>Use both</strong> if you are on Procore and can budget the combined cost — they integrate natively and solve non-overlapping problems.</p>", type: "text" }
    ],
  ...
  // ─── CompanyCam vs Raken ───────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      { heading: "Decision Framework", content: "<p><strong>Choose CompanyCam first</strong> if photos are getting lost, clients are asking for progress updates, or you need a searchable photo archive for insurance and dispute claims. At $49/month for 1 user (unlimited photos), it's the lower-friction starting point for most residential contractors. <strong>Choose Raken first</strong> if daily report quality is your primary risk — public work, federal contracts, or commercial projects where delay claims are real. Raken's free plan handles documentation for a single project. <strong>Use both</strong> when your contract revenue justifies the combined cost: the combination makes financial sense for contractors billing $750K+ per year, where a single defensible delay claim or avoided dispute pays for both tools in a month. For smaller operators, pick the tool that addresses your most immediate documentation gap and add the second in the next season.</p>", type: "text" }
    ],
