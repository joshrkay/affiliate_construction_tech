...
export function parseIntent(query) {
    ...
    if (BUDGET_SIGNALS.low.some((s) => q.includes(s)))
        budgetSignal = "low";
    else if (BUDGET_SIGNALS.mid.some((s) => q.includes(s)))
        budgetSignal = "mid";
    else if (BUDGET_SIGNALS.high.some((s) => q.includes(s)))
        budgetSignal = "high";
    // Team size
    let teamSizeSignal = "unknown";
    if (TEAM_SIZE_SIGNALS.solo.some((s) => q.includes(s)))
        teamSizeSignal = "solo";
    else if (TEAM_SIZE_SIGNALS.small.some((s) => q.includes(s)))
        teamSizeSignal = "small";
    else if (TEAM_SIZE_SIGNALS.medium.some((s) => q.includes(s)))
        teamSizeSignal = "medium";
    else if (TEAM_SIZE_SIGNALS.large.some((s) => q.includes(s)))
        teamSizeSignal = "large";
    // Category preference
    ...
    const reformattedQuery = tradeLabels.length > 0 || problemLabels.length > 0
        ? [
...
// ─── Scoring engine ───────────────────────────────────────────────────────────
function scoreToolForIntent(tool, intent, rawQuery) {
    ...
    // Category preference
    if (intent.categoryPreference && tool.category === intent.categoryPreference)
        score += 18;
    // Problem → feature mapping
    const PROBLEM_FEATURE_MAP = {
    ...
    for (const prob of intent.problemTypes) {
        ...
        const featureHits = featureKeywords.filter((kw) => tool.features.some((f) => f.toLowerCase().includes(kw)) ||
    ...
    if (intent.teamSizeSignal === "solo" || intent.teamSizeSignal === "small") {
        if (tool.companySize.includes("10") || tool.companySize.includes("50"))
            score += 8;
        if (tool.companySize.includes("1000"))
            score -= 6;
    }
    if (intent.teamSizeSignal === "large") {
        if (tool.companySize.includes("1000") || tool.companySize.includes("500"))
            score += 8;
    }
...
function generateHeadline(tool, intent) {
    ...
    if (intent.problemTypes.includes("scheduling"))
        return `Industry-leading scheduling & ${tool.type.toLowerCase()}`;
    if (intent.problemTypes.includes("estimating"))
        return `Proven ${tool.type.toLowerCase()} trusted by thousands`;
    if (intent.problemTypes.includes("invoicing"))
        return `Streamlined invoicing inside ${tool.name}`;
    return tool.tagline;
}
function generateExplanation(tool, intent, rawQuery) {
    const lines = [];
    const q = rawQuery.toLowerCase();
    // Trade context
    const tradeMatch = intent.tradeIds.filter((id) => tool.tradeIds.includes(id));
    ...
    if (lines.length < 2 && tool.pros.length > 0) {
        ...
        if (tool.pros.length > 1)
            lines.push(tool.pros[1]);
    }
    return lines.slice(0, 3).join(" ") || tool.tagline;
}
function generateCaution(tool, intent) {
    ...
    if (intent.budgetSignal === "low" && tool.price !== "Free") {
        const amount = parseFloat(tool.price.replace(/[^0-9.]/g, ""));
        if (amount > 250)
            return `Higher price point — verify ROI matches your current revenue.`;
    }
    // Trade mismatch
...
function generateBestFor(tool, intent) {
    if (intent.teamSizeSignal === "solo" || intent.teamSizeSignal === "small") {
...
function generateFollowUpQuestions(intent, topResults) {
    const questions = [];
    const topTool = topResults[0]?.tool;
    if (intent.tradeIds.length === 0)
        questions.push("Which trade are you in? (HVAC, electrical, roofing, etc.)");
    if (intent.problemTypes.length === 0)
        questions.push("What's the biggest operational challenge you're trying to solve?");
    if (intent.budgetSignal === "unknown")
        questions.push("What's your monthly software budget range?");
    if (intent.teamSizeSignal === "unknown")
        questions.push("How many techs or crew members do you manage?");
    if (topTool)
        questions.push(`How does ${topTool.name} compare to what you're currently using?`);
    if (intent.problemTypes.includes("estimating"))
        questions.push("Do you need the estimating to integrate with your project management?");
    if (intent.problemTypes.includes("scheduling"))
        questions.push("Do you also need customer-facing booking or just internal dispatch?");
    if (intent.tradeIds.includes("solar-renewable"))
        questions.push("Are you focused on residential solar, commercial, or both?");
    if (intent.tradeIds.includes("landscaping-irrigation"))
        questions.push("Do you need maintenance route management or primarily installation project tracking?");
    return questions.slice(0, 4);
}
function generateAnswerText(intent, topMatches, rawQuery) {
    ...
    const sizeContext = intent.teamSizeSignal !== "unknown"
...
function generateCategoryInsight(intent, matches) {
...
export function simulatedAISearch(rawQuery) {
    ...
    const scoredTools = tools
        ...
        .slice(0, 8);
    const toolMatches = scoredTools.map(({ tool, score }) => ({
        tool,
    ...
    const scoredTrades = trades
        .map((trade) => {
        let score = 0;
        if (intent.tradeIds.includes(trade.id))
            score += 40;
        const q = rawQuery.toLowerCase();
        if (trade.name.toLowerCase().split(" ").some((w) => q.includes(w)))
            score += 20;
        if (trade.tagline.toLowerCase().split(" ").some((w) => w.length > 3 && q.includes(w)))
            score += 10;
        return { trade, score };
    })
        .filter((r) => r.score > 10)
        .sort((a, b) => b.score - a.score)
        .slice(0, 3);
    const tradeMatches = scoredTrades.map(({ trade, score }) => ({
        trade,
...
export async function searchWithOpenAI(rawQuery, apiKey, onChunk) {
    ...
    const toolMatches = (parsed.recommendations || [])
        ...
        .filter((m) => m !== null);
    const tradeMatches = (parsed.trade_matches || [])
        .map((tm) => {
...
// ─── Main orchestrator ────────────────────────────────────────────────────────
export async function runAISearch(rawQuery, apiKey) {
    if (apiKey) {
        try {
