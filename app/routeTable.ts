...
export const routeTable: RouteDef[] = [
  ...
  // TEMP-DISABLED   { path: "blog", load: () => import("./pages/BlogIndexPage").then((m) => ({ Component: m.BlogIndexPage })) },
  // TEMP-DISABLED   { path: "blog/:slug", load: () => import("./pages/BlogPostPage").then((m) => ({ Component: m.BlogPostPage })) },
